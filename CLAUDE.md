...
CUDA is the primary target. Design every new algorithm so the hot path lives on the GPU, uses [[Memory module]] chunking, and avoids host↔device round-trips inside loops. **But CPU is a first-class fallback**: most users start there and many fMRI workflows live on CPU permanently. The expectation is **rough parity with the equivalent reference tool on CPU**, not "CPU is the slow path we don't care about". MPS is a supported default on Apple Silicon (torch >= 2.14): `get_device` picks it automatically, unimplemented ops fall back to the CPU via `PYTORCH_ENABLE_MPS_FALLBACK`, and the ops where Metal is broken or slower than Apple's CPU are routed off it by `utils.py:cpu_if_mps` against the measured table `utils.py:_MPS_CPU_OPS`. **Never hand-write `device.type == "mps"` to pick a device** — add a row to that table instead, and re-measure with `scripts/bench_mps_policy.py` after a torch upgrade. float64 is the one hard Metal limit and has its own routing (`linalg_device` / `accum_dtype`); `-device cpu` stays fully supported for guaranteed float64.
...
- [[Memory module]] — `fastfuncstuff/memory.py` is the single source of truth for chunk sizing. Every GPU-accelerated path must call `memory.py:estimate_chunk_size` (or a `bytes_per_voxel_*` planner); never hardcode. The 0.5 GPU safety factor compensates for PyTorch's caching allocator.
...
- **`-device` goes through one parser.** Register the flag with `cli_utils.py:add_device_arg` and resolve it with `cli_utils.py:setup_device` (which calls `parse_device_arg` + `configure_torch_backends`). Never `torch.device(args.device)` — bare torch rejects the documented `cuda,N` / `cpu,N` spec forms, so a hand-rolled parse silently drops the ability to pin a card or cap threads. Registration tools pass `tf32=REGISTRATION_TF32`.
...
- **CLI flags**: AFNI-style single-dash long flags (`-hrf-library`, `-lambda-mode`). Build the parser with `cli_help.py:FfsArgumentParser` — it derives the `-foo_bar` / `-foo-bar` twin of every flag automatically (hidden from `-help` and from completion, which show one spelling), and it carries `FfsHelpFormatter` as its default. Never `argparse.ArgumentParser` directly; the toolbox spent a long time with 58% of its multi-word flags accepting only one spelling.
- **Help text**: one formatter, `cli_help.py:FfsHelpFormatter`, for every tool — newlines in help survive, epilogs keep their layout, defaults print once and never as `None`/`False`. Don't hand-roll a `_HelpFormatter`; there used to be 23 of them.
- **Completions**: static, generated by `ffs_completion` (`make completions`). Regenerate after adding or renaming a flag. A value set that `choices=` would wrongly close — `-device`, `-work_dxyz` — goes through `cli_help.py:suggest`, which offers without enforcing.
