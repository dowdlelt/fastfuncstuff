...
from fastfuncstuff.cli.task_events import (
    psc_betas,
    save_labelled_map,
    save_task_fit,
    save_task_map,
    task_design_from_events,
)
...
def _paired_bins_for(args, n_timepoints: int, tr: float, device):
    ...
    design, labels = task_design_from_events(args, n_timepoints, tr, device)
...
def _write_task_diagnostics(result, data, stem, ext, affine, args, tr):
    ...
    for path, arr, brick_labels, stat_polort in pending:
        if stat_polort is None:  # the rms map: map units, no stat to attach
            save_labelled_map(path, arr, brick_labels, affine)
        else:
            save_task_map(path, arr, brick_labels, affine, n_ort=stat_polort + 1, n_t=n_t)
    ...
    psc_before = psc_betas(data_tc, design, reference, mask)
    save_task_fit(f"{stem}_taskfit_data{ext}", data_tc, psc_before, labels, affine, polort + 1, n_t)
...
def _write_task_after(result, design, polort, resp, mask, stem, ext, affine, args):
    ...
    for path, arr in pending:
        save_task_map(path, arr, args._task_labels, affine, n_ort=polort + 1, n_t=n_t)
...
def _write_data_task_after(corrected, stem, args, pc_scores=None, pc_note=""):
    ...
    psc_after = psc_betas(after, st["design"], st["reference"], st["mask"])
    ...
    psc_pcs = psc_betas(with_pcs, st["design"], st["reference"], st["mask"])
