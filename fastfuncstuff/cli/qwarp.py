...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    g_interp.add_argument(
        "-final",
        choices=list(WARP_INTERP_MODES),
        default="wsinc5",
    ...
    g_disp.add_argument(
        "-noXdis", action="store_true", help="Disable displacement along X (left-right)"
    )
    g_disp.add_argument(
        "-noYdis", action="store_true", help="Disable displacement along Y (anterior-posterior)"
    )
    g_disp.add_argument(
        "-noZdis", action="store_true", help="Disable displacement along Z (inferior-superior)"
    )
    g_disp.add_argument(
        "-axweight",
        nargs=3,
    ...
    g_disp.add_argument(
        ...
        default=0.0,
        metavar="VOXELS",
        help="Hard limit on maximum displacement in voxels. 0 = no limit [default: %(default)s]",
    )
    g_disp.add_argument(
        "-motparams",
        type=str,
    ...
    g_cost.add_argument("-pear", action="store_true", help="Plain (unclipped) Pearson correlation")
    g_cost.add_argument(
        "-lpa",
        action="store_true",
    ...
    g_wt.add_argument(
        ...
        default=None,
        metavar="FILE.nii.gz",
        help="Save the computed autoweight image to this file. Only used with -autoweight.",
    )
...
def _pe_direction_from_flags(
    noXdis: bool, noYdis: bool, noZdis: bool
...
def _extract_warp_pcs(
    ...
    if verb >= 1:
        ...
        print(
            f"Warp PCs ({axes_str}, {n_vols} vols): extracted {n_pcs} PCs, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if timeseries_mode:
        if source_is_4d:
            ...
            nt = nt_full
            if args.verb >= 1:
                print(f"Timeseries mode (external base): {nt_full} source volumes, {nx}x{ny}x{nz}")
        else:
            if base_data.ndim != 4:
            ...
            if args.tprange is not None:
                ...
                if args.verb >= 1:
                    print(
                        f"Timepoint selection: {nt_orig} -> {base_data.shape[0]} volumes (slice {sl})"
                    )
        ...
        if base_data.ndim == 4:
            if args.base_index is not None:
                if args.base_index < 0 or args.base_index >= base_data.shape[0]:
                    print(
                        f"ERROR: -base_index {args.base_index} out of range "
                ...
                if args.verb >= 1:
                    print(
                        f"Using base vol[{args.base_index}] from 4D ({base_data.shape[0]} volumes)"
                    )
            else:
                base_3d = base_data[0]
                if args.verb >= 1:
                    print(
                        "WARNING: 4D base with -source; using vol[0]. "
    ...
    if args.lpa_kernel == "box" and lpa_sigma_val <= 0:
        ...
        if args.verb >= 1:
            ...
            print(f"Auto box radius: {int(lpa_sigma_val)} ({side}³ = {side**3} voxels)")
    ...
    if args.motparams is not None:
        ...
        if args.verb >= 1:
            ...
            print(
                f"Motion-projected distortion: PE along {'+'.join(pe_labels)}, "
    ...
    if args.affine is not None:
        ...
        if args.verb >= 1:
            ...
            print(
                f"Affine-projected distortion: PE along {'+'.join(pe_labels)}, "
    ...
    if args.autoweight:
        ...
        if args.verb >= 1:
            ...
            print(
                f"Autoweight: automask + smooth base(sigma={args.autoweight_blur:.1f}), "
                f"{n_nonzero}/{n_total} nonzero voxels "
                f"({100 * n_nonzero / n_total:.1f}%)"
            )
        if args.save_autoweight is not None:
            with spinner(f"Writing {Path(args.save_autoweight).name}"):
        ...
        for i, src_idx in enumerate(source_indices):
            ...
            if chain_warps and len(warp_buffer) > 0:
                if len(warp_buffer) == 1:
                    ...
                    if args.verb >= 1:
                        print(
                            f"  Lookback: best init from {chosen_label} "
            ...
            for i, src_idx in enumerate(source_indices):
                ...
                if not args.no_save_warp:
                    save_warp_field(
                        ...
                        szd,
                        os.path.join(
                            warp_dir, f"{warp_basename}_WARPsmooth_t{src_idx:0{zpad}d}.nii.gz"
                        ),
                        header_info=base_info,
                        padding=warp_padding,
                        units="mm",
