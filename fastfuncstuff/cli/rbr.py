...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = FfsArgumentParser(
        ...
        epilog=(
            ...
            "TUNING BY SYMPTOM\n"
            "-----------------\n"
            "Read the run's summary line first: it reports the cost before and after,\n"
            "the fraction of boundary points that already had the right contrast sign,\n"
            "and max |disp| in voxels. A low correct-sign start means the affine or the\n"
            "polarity is wrong, not the warp; fix that before touching any knob here.\n"
            "\n"
            "Not enough correction (EPI still pulled off the boundary):\n"
            "  * Finer control grid: -spacings 16 8 4 (or add a finer level). Spacing is\n"
            "    in voxels and is the wavelength of the deformation you can express.\n"
            "  * Lower -reg_weight (1.0 -> 0.3): the membrane term shrinks amplitude, so\n"
            "    it is the usual reason a correction looks half-applied.\n"
            "  * Widen the capture range on the coarse levels: -offsets 6 4 3 samples the\n"
            "    EPI farther either side of the boundary, which sees bigger errors.\n"
            "  * Raise -iters / -lr if the cost was still falling when the level ended.\n"
            "  * A whole region is ignored: dropout there has no grey/white edge and gets\n"
            "    down-weighted by design (the run prints what fraction). Add -target edges\n"
            "    (with -anat) or -target tissue so something else constrains it. Do not\n"
            "    reach for -no_reliability first -- that lets noise drive the fit.\n"
            "\n"
            "Too much correction (anatomy stretched, wavy boundary, silly max |disp|):\n"
            "  * Coarser -spacings and/or higher -reg_weight -- in that order.\n"
            "  * Keep the PE-axis-only default. -full3d triples the freedom and drops the\n"
            "    physical prior that distortion is ~1-D; it overfits readily.\n"
            "  * Narrower -offsets: wide sampling on fine levels chases distant structure.\n"
            "  * Combining targets: -ngf_weight / -tissue_weight scale the edge and\n"
            "    partial-volume terms against the WM BBR term. One target dominating is a\n"
            "    weighting problem, not a regularization one.\n"
        ),
    )
