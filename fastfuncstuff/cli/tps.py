...
def main():
    ...
    # Stimulus timing
    parser.add_argument(
        "-stim-times", nargs="+", required=True, help="Onset time files (one per condition)"
    )
    parser.add_argument(
        "-stim-labels",
        nargs="+",
    ...
    parser.add_argument(
        "-force-zero-edges", action="store_true", help="Force HRF to be zero at window start/end"
    )

    # Cross-validation
    parser.add_argument(
        "-optimize-level",
        choices=["global", "per_voxel"],
        default="global",
    ...
    # Nuisance regressors
    parser.add_argument(
        "-polort", type=int, default=3, help="Polynomial detrending order (-1 = none). Default: 3"
    )
    ...
    # Output options
    parser.add_argument("-save-design", action="store_true", help="Save design matrix as .1D file")
    parser.add_argument(
        "-save-lambda-map",
        action="store_true",
    ...
    if len(args.stim_times) != len(args.stim_labels):
        parser.error(
            f"Number of -stim_times ({len(args.stim_times)}) must match "
    ...
    for cond_idx, onset_file in enumerate(args.stim_times):
        ...
        if n_runs is None:
            ...
            raise ValueError(
                f"Condition '{label}' has {len(onsets)} runs, but first condition has {n_runs} runs"
            )
    ...
    if args.polort >= 0:
        ...
        if args.verb >= 1:
            print(
                f"  Polynomial regressors: {total_poly_cols} ({n_poly_per_run} per run x {n_runs} runs)"
            )
    else:
    ...
    if args.verb >= 1:
        ...
        print(
            f"  Penalizes {n_stimulus_regressors} stimulus regressors across {n_conditions} conditions"
        )
    ...
    if total_poly_cols > 0:
        ...
        pred_stimulus = (
            design_stimulus @ betas_stimulus_tensor.T.to(device)
