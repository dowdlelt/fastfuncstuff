...
# ---------------------------------------------------------------------------
# A completion description is one line, and every shell's pager cuts it to the
# terminal width -- there is no way to expand an entry in place. So the full
# help of every flag is written out as plain text, one file per flag, and a
# key binding prints the one under the cursor. Files rather than calling
# `<tool> -help`: a CLI takes ~2 s to start, a key press should not.


def flag_help_key(option: str) -> str:
    """The filename stem a flag's help is stored under: every spelling of one
    flag (-foo_bar / -foo-bar) lands on the same file, and the shell side can
    normalise whatever the user typed the same way."""
    return _norm(option)


def flag_help_texts(parser: argparse.ArgumentParser, width: int = 100) -> dict[str, str]:
    """``{flag_help_key: help block}`` for every offered flag, rendered exactly as
    ``-help`` renders that one entry (flag line, wrapped text, default)."""
    out: dict[str, str] = {}
    for action in parser._actions:
        if not action.option_strings or action.help is argparse.SUPPRESS:
            continue
        # A fixed width, not the terminal's: these files are written once, offline.
        fmt_class = cast("type[argparse.HelpFormatter]", parser.formatter_class)
        try:
            fmt = fmt_class(prog=parser.prog, width=width)
        except TypeError:  # a formatter that does not take width
            fmt = parser._get_formatter()
        fmt.add_argument(action)
        text = fmt.format_help().rstrip() + "\n"
        for option in action.option_strings:
            # A flag registered twice (see describe) keeps its first entry.
            out.setdefault(flag_help_key(option), text)
    return out


def render_fish_help_key(help_dir: str) -> str:
    """A fish conf.d snippet binding alt-h to the full help of the ffs_* flag at
    the cursor. On any other command alt-h keeps its stock man-page lookup."""
    quoted = _shell_quote(help_dir)
    return f"""# ffs_* flag help on a key -- generated by ffs_completion (make completions).
# alt-h on an ffs_* command line prints the FULL -help entry for the flag under
# the cursor, or for the flag whose value the cursor is on. A partial flag lists
# the flags it could be. On any other command alt-h is fish's man-page lookup.
set -g __ffs_flag_help_dir '{quoted}'

# The lookup, apart from the key binding so it can run (and be tested) without
# an interactive command line: TOOL, the token at the cursor, then the tokens
# before it.
function __ffs_flag_help_lookup --argument-names cmd tok
    set -l before $argv[3..-1]
    set -l dir $__ffs_flag_help_dir/$cmd
    ...
    set -l key (string replace -ra -- '[-_]' '' "$tok")
    if test -z "$key"
        echo "$cmd: put the cursor on a flag (or on its value) for its help."
        return 1
    end
    if test -f $dir/$key.txt
        cat $dir/$key.txt
        return 0
    end
    set -l hits $dir/$key*.txt
    if test (count $hits) -eq 0
        echo "$cmd: no flag matches $tok"
        return 1
    end
    echo "$cmd:" (count $hits) "flags start with $tok:"
    for h in $hits
        head -n 1 $h
    end
end

function __ffs_flag_help --description 'Full -help text of the ffs_* flag at the cursor'
    set -l cmd (commandline -po)[1]
    if not string match -q 'ffs_*' -- "$cmd"; or not test -d $__ffs_flag_help_dir/$cmd
        __fish_man_page
        return
    end
    echo
    __ffs_flag_help_lookup $cmd (commandline -t) (commandline -opc)[2..-1]
    commandline -f repaint
end

bind \eh __ffs_flag_help
bind -M insert \eh __ffs_flag_help
"""


# bash and zsh share one POSIX lookup, called with the help dir, the tool, the
# token at the cursor and the tokens before it. fish has its own native copy of
# the same logic in render_fish_help_key.
HELP_LOOKUP_SCRIPT = "lookup.sh"

_HELP_LOOKUP_SH = r"""#!/bin/sh
# ffs_* flag help lookup -- generated by ffs_completion; the bash/zsh alt-h
# bindings call it. usage: lookup.sh HELP_DIR TOOL TOKEN [TOKENS BEFORE CURSOR...]
dir=$1/$2; cmd=$2; tok=$3; shift 3
case $tok in
-*) ;;
*)  # On a value: explain the nearest flag before it.
    tok=
    for t in "$@"; do
        case $t in -*) tok=$t ;; esac
    done ;;
esac
key=$(printf '%s' "$tok" | tr -d '_-')
if [ -z "$key" ]; then
    echo "$cmd: put the cursor on a flag (or on its value) for its help."
    exit 1
fi
if [ -f "$dir/$key.txt" ]; then
    cat "$dir/$key.txt"
    exit 0
fi
set -- "$dir/$key"*.txt
...
echo "$cmd: $# flags start with $tok:"
for h in "$@"; do head -n 1 "$h"; done
"""


def render_help_lookup_sh() -> str:
    return _HELP_LOOKUP_SH


def render_bash_help_key(help_dir: str) -> str:
    """A bash snippet (source it from ~/.bashrc) binding alt-h to the full help of
    the ffs_* flag at the cursor. alt-h is unbound in stock bash."""
    quoted = _shell_quote(help_dir)
    return f"""# ffs_* flag help on a key -- generated by ffs_completion (make completions).
# Source from ~/.bashrc. alt-h on an ffs_* command line prints the FULL -help
# entry for the flag under the cursor, or for the flag whose value the cursor is
# on. A partial flag lists the flags it could be.
__FFS_FLAG_HELP_DIR='{quoted}'

__ffs_flag_help() {{
    local left=${{READLINE_LINE:0:READLINE_POINT}} right=${{READLINE_LINE:READLINE_POINT}}
    local -a all words
    read -ra all <<< "$READLINE_LINE"
    read -ra words <<< "$left"
    local cmd=${{all[0]}} tok=
    [[ $cmd == ffs_* && -d $__FFS_FLAG_HELP_DIR/$cmd ]] || return 0
    # The token under the cursor: the tail of what is left of it, plus the head
    # of what is right of it.
    if [[ -n $left && $left != *[[:space:]] && ${{#words[@]}} -gt 0 ]]; then
        tok=${{words[-1]}}
        unset 'words[-1]'
    fi
    tok+=${{right%%[[:space:]]*}}
    echo
    sh "$__FFS_FLAG_HELP_DIR/{HELP_LOOKUP_SCRIPT}" "$__FFS_FLAG_HELP_DIR" "$cmd" "$tok" "${{words[@]:1}}"
}}

bind -x '"\eh": __ffs_flag_help'
bind -m vi-insert -x '"\eh": __ffs_flag_help'
"""


def render_zsh_help_key(help_dir: str) -> str:
    """A zsh snippet (source it from ~/.zshrc) binding alt-h to the full help of
    the ffs_* flag at the cursor. On any other command alt-h keeps zsh's stock
    run-help."""
    quoted = _shell_quote(help_dir)
    return f"""# ffs_* flag help on a key -- generated by ffs_completion (make completions).
# Source from ~/.zshrc. alt-h on an ffs_* command line prints the FULL -help
# entry for the flag under the cursor, or for the flag whose value the cursor is
# on. A partial flag lists the flags it could be. On any other command alt-h is
# zsh's run-help.
typeset -g __FFS_FLAG_HELP_DIR='{quoted}'
...
__ffs_flag_help() {{
    ...
    words=(${{(z)LBUFFER}})
    local cmd=${{all[1]}} tok=
    if [[ $cmd != ffs_* || ! -d $__FFS_FLAG_HELP_DIR/$cmd ]]; then
        zle run-help
        return
    fi
    # The token under the cursor: the tail of what is left of it, plus the head
    # of what is right of it.
    if [[ -n $LBUFFER && $LBUFFER != *[[:space:]] && ${{#words}} -gt 0 ]]; then
        tok=${{words[-1]}}
        words[-1]=()
    fi
    tok+=${{RBUFFER%%[[:space:]]*}}
    zle -I
    print
    sh "$__FFS_FLAG_HELP_DIR/{HELP_LOOKUP_SCRIPT}" "$__FFS_FLAG_HELP_DIR" "$cmd" "$tok" "${{(@)words[2,-1]}}"
}}

zle -N __ffs_flag_help
bindkey '^[h' __ffs_flag_help
bindkey -M viins '^[h' __ffs_flag_help
"""


HELP_KEY_RENDERERS = {
    "bash": render_bash_help_key,
    "fish": render_fish_help_key,
    "zsh": render_zsh_help_key,
}


def write_flag_help(outdir, prog: str, parser: argparse.ArgumentParser) -> int:
    """Write ``<outdir>/<prog>/<key>.txt`` for every flag; returns the count.

    The tool's directory is cleared first, so a renamed or removed flag stops
    answering instead of serving its old text forever."""
    from pathlib import Path

    tool_dir = Path(outdir) / prog
    tool_dir.mkdir(parents=True, exist_ok=True)
    for old in tool_dir.glob("*.txt"):
        old.unlink()
    texts = flag_help_texts(parser)
    for key, text in texts.items():
        (tool_dir / f"{key}.txt").write_text(text)
    return len(texts)
