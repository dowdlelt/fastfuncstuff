...
results, design_info = ffs.analyze_from_design_matrix(
    ...
    precomputed_arma_params=precomputed_arma,  # ← Use precomputed if available
    device=device,
)
...
# Save ARMA (a, b) parameters
arma_params_path = data_dir / "arma_params.nii.gz"
if precomputed_arma is not None:
    print(f"\nARMA parameters already exist: {arma_params_path.name}")
    print("  Skipping save (parameters unchanged)")
else:
    print(f"\nSaving ARMA parameters to: {arma_params_path.name}")
    ...
    print("✓ Saved ARMA parameters (2 volumes: a, b)")
    print(f"  Mean a: {results.arma_params[:, 0].mean():.3f}")
    print(f"  Mean b: {results.arma_params[:, 1].mean():.3f}")
    print("\n  → Next run will use these parameters for 80% time savings!")

# Save lambda (lag-1 autocorrelation)
lambda_path = data_dir / "arma_lambda.nii.gz"
print(f"\nSaving ARMA λ (lag-1 autocorrelation) to: {lambda_path.name}")

# Reshape lambda to volume
if hasattr(results, "voxel_mask") and results.voxel_mask is not None:
    mask_flat = results.voxel_mask.cpu().numpy().reshape(-1)
    lambda_vol = np.zeros(np.prod(results.full_shape), dtype=np.float32)
    lambda_vol[mask_flat] = results.arma_lambda.cpu().numpy()
    lambda_vol = lambda_vol.reshape(results.full_shape)
else:
    lambda_vol = results.arma_lambda.cpu().numpy().reshape(results.full_shape)

lambda_img = nib.Nifti1Image(lambda_vol, results.affine)
lambda_img.header.set_xyzt_units(xyz="mm")
lambda_img.header["descrip"] = b"ARMA(1,1) lambda: lag-1 autocorrelation"
nib.save(lambda_img, lambda_path)
print(f"✓ Saved λ parameter")
...
Output Files:
  ...
  2. arma_params.nii.gz - ARMA(1,1) parameters (like AFNI 3dREMLfit)
     • Volume 0: a parameter (AR component)
     • Volume 1: b parameter (MA component)
     • Reusable for fast re-analysis (80% time savings!)
  
  3. arma_lambda.nii.gz - Lag-1 autocorrelation
     • λ = effective temporal correlation
