...
def test_pca_vs_sklearn(n_samples, n_features, n_components):
    ...
    # FastFuncSim PCA
    ffs_pca = PCA(
        n_components=n_components, device=torch.device("cpu")
    )  # Test CPU first for strict numerical match
    ...
    np.testing.assert_allclose(
        ffs_pca.explained_variance_ratio_.numpy(), sk_pca.explained_variance_ratio_, atol=1e-5
    )
    ...
    assert np.mean(score_corrs) > 0.99, (
        f"PCA Scores do not match sklearn (mean corr={np.mean(score_corrs)})"
    )
...
def test_ica_spatial_recovery():
    ...
    assert mean_corr_temporal > 0.90, (
        f"Failed to recover mixing timecourses (corr={mean_corr_temporal})"
    )
