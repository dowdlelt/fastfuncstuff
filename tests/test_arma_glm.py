...
def test_reml_grid_search():
    ...
    noise = generate_arma_noise(
        ar_coeffs=[true_a], ma_coeffs=[true_b], n_timepoints=n_timepoints, n_voxels=1, device=device
    ).squeeze()
...
def test_prewhitening():
    ...
    X_w_manual = torch.linalg.solve_triangular(L_inv, X, upper=False)

    match = torch.allclose(X_w, X_w_manual, atol=1e-5)
    print(f"\nPrewhitening matches manual computation: {match}")

    assert match, "Prewhitening should match manual L^{{-1}} X via triangular solve"
...
def test_glm_arma11():
    ...
    print(
        f"  ARMA mean (a, b): ({arma_results.arma_params[:, 0].mean():.3f}, "
...
def test_batch_processing():
    ...
    for v in range(n_voxels_batch):
        ...
        noise_v = generate_arma_noise(
            ar_coeffs=[a_v], ma_coeffs=[b_v], n_timepoints=n_timepoints, n_voxels=1, device=device
        ).squeeze()
