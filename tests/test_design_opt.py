...
class TestGenerateEventSequence:
    def test_alternating_order(self):
        seq = generate_event_sequence(
            n_trials_per_condition=5, n_conditions=2,
            ordering="alternating", seed=42,
        )
        assert len(seq) == 10
        assert set(seq) == {0, 1}
    ...
    def test_random_order_has_all_conditions(self):
        seq = generate_event_sequence(
            n_trials_per_condition=8, n_conditions=3,
            ordering="random", seed=42,
        )
        assert len(seq) == 24
    ...
    def test_balanced_counts(self):
        seq = generate_event_sequence(
            n_trials_per_condition=10, n_conditions=4,
            ordering="random", seed=0,
        )
        for c in range(4):
...
class TestGenerateISISequence:
    def test_exponential_distribution(self):
        ...
        isis = generate_isi_sequence(
            n_events=50, isi_constraints=constraints,
            distribution="exponential", seed=42,
        )
    ...
    def test_uniform_distribution(self):
        ...
        isis = generate_isi_sequence(
            n_events=30, isi_constraints=constraints,
...
class TestCreateOnsetMatrix:
    def test_basic_onset_matrix(self):
        ...
        isis = generate_isi_sequence(
            n_events=10, isi_constraints=constraints,
            distribution="exponential", seed=42,
        )
        onsets = create_onset_matrix(
            event_sequence=seq, isis=isis, duration=100.0,
            tr=1.0, n_conditions=2,
        )
