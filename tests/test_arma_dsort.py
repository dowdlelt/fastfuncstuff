...
def synthetic(rng=np.random.default_rng(0)):
    ...
    return dict(
        device=device, X=X, p=p, q=q, n_time=n_time, n_vox=n_vox,
        dsort=dsort, beta=beta, gamma=gamma, data=data,
    )
...
def test_nods_snapshot_matches_plain_fit(synthetic):
    s = synthetic
    common = dict(
        tr=2.0, device=s["device"], verbose=False, use_double=True,
    )
    res = fit_glm_arma11(
        torch.from_numpy(s["data"]),
        torch.from_numpy(s["X"]),
    ...
    assert np.allclose(
        res.nods_results.betas.numpy(), plain.betas.numpy(), atol=1e-4, rtol=1e-3
    )
