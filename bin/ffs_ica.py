...
def _load_run_ortvec_design(
    ortvec_specs: list[list[str]] | None,
    run_idx: int,
    n_timepoints: int,
    device: torch.device,
) -> tuple[torch.Tensor | None, list[str] | None]:
    """Load run-specific ortvec regressors from CLI specs.

    Each spec is: [LABEL, file1, file2, ...]. For multi-run input,
    file list can be either length 1 (shared) or length n_runs.
    """
    if not ortvec_specs:
        return None, None
...
def _expand_mask_file(mask_path: str, shape3d: tuple[int, int, int]) -> list[tuple[str, np.ndarray]]:
    ...
    # If >1 positive integer labels, treat as label map with one mask per label.
...
def _prepare_guidance_masks(
    ...
    """Parse good/bad masks and map them into masked voxel index space."""
    if not mask_paths:
        return []

    out: list[dict] = []
    for path in mask_paths:
        expanded = _expand_mask_file(path, shape3d)
...
def _mean_abs_by_selector(comp_kv: np.ndarray, selector_v: np.ndarray) -> np.ndarray:
...
def _mean_z_excess_by_selector(z_kv: np.ndarray, selector_v: np.ndarray, z_thresh: float) -> np.ndarray:
    if int(selector_v.sum()) == 0:
...
def _best_lag_and_r(
    ...
    Tries rapidtide helpers if available; otherwise uses a normalized
    numpy cross-correlation fallback.
    """
    ...
    # Attempt rapidtide integration first (API can vary by release).
    try:
    ...
    # Robust fallback: z-score and normalized xcorr over bounded lags.
    x = x - np.mean(x)
    y = y - np.mean(y)
...
def _weighted_depth_timeseries(
...
def _save_corr_heatmap(corr_kn: np.ndarray, labels: list[str], out_png: Path, title: str):
...
def _save_score_heatmap(scores_kn: np.ndarray, labels: list[str], out_png: Path, title: str, cmap: str):
...
def _preprocess_design_for_correlation(
    ...
    """Apply the same temporal preprocessing to regressors used for correlation.

    ICA timecourses come from data that has already undergone polort projection
    and optional high-pass filtering, so we transform condition/ortvec vectors
    the same way before correlation.
    """
    if design_tc.numel() == 0:
        return design_tc

    # Convert (T, C) -> (C, T) to match utility function conventions.
    reg_ct = design_tc.T
...
def _component_condition_spectral_correlations(
    mixing_tk: torch.Tensor,
    design_tc: torch.Tensor,
) -> np.ndarray:
    """Correlation between component and design power spectra (per column pair).

    Uses rFFT power (excluding DC), z-scores each spectrum across frequency,
    then computes Pearson-like cosine in standardized space.
    """
    x = mixing_tk.detach().cpu().numpy().astype(np.float64)  # (T, K)
    y = design_tc.detach().cpu().numpy().astype(np.float64)  # (T, C)

    sx = np.abs(np.fft.rfft(x, axis=0)) ** 2  # (F, K)
    sy = np.abs(np.fft.rfft(y, axis=0)) ** 2  # (F, C)

    # Drop DC to avoid mean-offset domination.
    if sx.shape[0] > 1:
        sx = sx[1:, :]
        sy = sy[1:, :]
    ...
    # Log-compress power for numerical stability before standardization.
    sx = np.log1p(sx)
    sy = np.log1p(sy)
